...
4. TASK 15 之前不得宣称 Phase 11 完成。
5. 每个主干 TASK 完成后必须运行 `python Plugins/AgentBridge/Scripts/validation/validate_examples.py --strict`。
6. `Docs/Phase11/` 下每个新增功能都必须在本文件的“Phase 11 文档包功能覆盖矩阵”中有 TASK 落点。
7. TASK 15 最终验收必须输出 Phase 11 文档包功能覆盖报告，逐项说明已覆盖、证据文件、残留风险。
...
| [01_Phase11_Unified_Architecture.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/Phase11/01_Phase11_Unified_Architecture.md) | 7 阶段主链、MCP 前后端边界、50 工具目标、v1/v2 兼容、Run 隔离、GDD-First 命名 | TASK 02、TASK 03、TASK 05、TASK 10、TASK 12、TASK 15 | Session/Orchestrator/MCP/Final 验收报告 |
...
| [03_Universal_Baseline_Standard.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/Phase11/03_Universal_Baseline_Standard.md) | Universal Baseline 分类、presence_only 最低标准、Settings 六项底线、realization_eligible 升级规则 | TASK 06、TASK 09B、TASK 10、TASK 11、TASK 14、TASK 15 | `baseline_coverage_report`、TASK 14 报告 |
| [04_Baseline_Realization_Policy.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/Phase11/04_Baseline_Realization_Policy.md) | Baseline 三类域、模板结构、Baseline Fragment、Start Screen 能力级定义 | TASK 06、TASK 08、TASK 09B、TASK 14 | Baseline Fragment 与模板报告 |
...
| [08_Skill_Graph_and_Domain_Skill.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/Phase11/08_Skill_Graph_and_Domain_Skill.md) | Skill Graph 节点/边、Domain Skill 四重职责、Baseline/GamePlay 同构、SkillTemplates vs Skills、生命周期 | TASK 08、TASK 09B、TASK 14 | `skill_graph.json`、runtime 生命周期报告 |
...
| M6 | LLM Client 接入 + Domain Skill Runtime + Design Space / Realization / Convergence | TASK 09A-09B | 🔄 需要重新验收 |
| M7 | Cross Review v2 + Build IR v2 + Handoff v3 | TASK 10 | ⏳ 待执行 |
| M8 | Monopoly GDD v2 端到端 run 与最小可玩闭环验收 | TASK 11 | ⏳ 待执行 |
| M9 | Defer 治理项：run compare/promote、完整 fast_mode、Baseline Template 全套 | TASK 12-14 | ⏳ 待执行 |
| M10 | 最终验收、证据归档、文档收尾 | TASK 15 | ⏳ 待执行 |
...
- MCP 工具总数达到 Phase 11 目标 50（原 42 + 新增 8：root_skill prepare/save、clarification prepare/save、skill_graph prepare/save、stage4_node prepare/save — intake/plan 为旧名 alias 不计入新增），或报告解释差异。实际 TOOL_DISPATCH 计数为 50。
- `validate_examples.py --strict` 通过。
- 证据报告存在，并在成功声明中附链接。
...
状态：🔄 需要重新验收  
旧证据：[task09_phase11_domain_skill_runtime.md](/D:/UnrealProjects/Mvpv4TestCodex/ProjectState/Reports/2026-04-15/task09_phase11_domain_skill_runtime.md)（不完整，缺 LLM Internal 和 MCP Agent 路径验证）

> **重新验收原因**：原 TASK 09 在 llm_client 未实现、MCP 交互工具未创建时由 Codex 标记完成。Claude Code 已补充 MCP 交互工具和 generator_provider 字段，但 LLM Internal 路径（TASK 09A）仍未接入。三路 Generator 验收需要 TASK 09A 先完成。
> 
> **已完成的部分**（由 Codex + Claude Code 完成，不需重做）：
> - `agent_protocol.py`（GeneratorProvider 抽象 + LLMProvider + HeuristicFallbackProvider + AgentPhaseRunner）
> - `discovery_fallback.py` / `realization_fallback.py` / `convergence_fallback.py`
> - `domain_skill_runtime.py`（含 prepare_node_phase / save_node_phase MCP 交互入口）
> - MCP 工具 `compiler_stage4_node_prepare` / `compiler_stage4_node_save`（tool_definitions + server + compiler_tools）
> - `pipeline_orchestrator.py` Stage 4 路由 + generator_provider 记录
> - `session.py` generator_provider 字段 + is_promotable 属性
> 
> **需要重新执行的部分**：
> - 使用 LLM Internal 路径（TASK 09A 的 llm_client）运行一次完整 Stage 4，验证 LLMProvider 产出
> - 使用 MCP Agent 路径运行一次 prepare → save 循环，验证 MCP 交互模式
> - 使用 Heuristic Fallback 路径运行一次，验证降级标记
> - 更新证据报告，覆盖三路 Generator 的完整验收
...
9. 运行三路 Generator 验证：
   - 9a. LLM Internal 路径：配置 `llm_config.yaml`，运行 Stage 4，验证 `generator_type: "llm"` + `promotable: true`。
...
- **三种 Generator 路径均已验证**：证据报告必须包含三路各至少一次成功运行的记录和产物对比。
- `validate_examples.py --strict` 通过。
- 证据报告存在，并在成功声明中附链接。
