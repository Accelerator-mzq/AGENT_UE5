# MCP Tools Catalog — 58 工具字段级签名表

> 版本: v1.3(2026-06-13,Phase 15 新增 `demo_feedback_log` 反馈登记工具,57→58);v1.2(2026-06-12,Phase 14 新增 `demo_story_fetch/submit` demo story 工具对,55→57;2026-06-11 Phase 13 新增 `compiler_skill_synthesis_prepare/save` 合成工具对,53→55)
...
> 权威源: Plugins/AgentBridge/MCP/tool_definitions.py(`len(ALL_TOOLS) == 58`,Python 实测)
> 数字注脚: **Python `len()` 实测 58**(Bridge 28 + 前端 19 + 后端 11;demo 工具对注册在 `COMPILER_FRONTEND_TOOLS` 内,16→18→19),以 `len()` 实测为准;tool_definitions.py:565 分类合计注释已与实测一致
...
| demo_feedback_log | 前端 Compiler | n/a — 见 tool_definitions.py(session_path 必填 run 目录;entry 对象:window_id/phenomenon/expectation/severity/related_rung/related_capability,feedback_id 与 status 由工具生成) | n/a — data.feedback_id / data.path;条目 schema 不合法 → status=failed | INVALID_ARGS, TOOL_EXECUTION_FAILED | P15 反馈回流登记 |
...
- 主表实测行数: 58
- 类别分布:Bridge L1 查询 7 / L1 写 6 / L1 服务 5 / L2 资产 9 / L3 兜底 1 / 前端 19 / 后端 11 = 58 ✓
- 自检命令:`awk -F'|' 'NF != 8 && /^\|/ && !/^\| 工具名/ && !/^\| ---/' Docs/contracts/mcp_tools_catalog.md`(应空)
- Python 实测命令:`python -c "from tool_definitions import ALL_TOOLS; print(len(ALL_TOOLS))"` → `58`
- 口径历史:Phase 11 收尾实测 53(曾与 line 530 注释"51"冲突,源码注释已修);Phase 13(2026-06-11)新增合成工具对后实测 55;Phase 14(2026-06-12)`COMPILER_FRONTEND_TOOLS` 增 `demo_story_fetch/submit` 工具对后实测 **57**,tool_definitions.py:565 分类合计注释与实测一致;Phase 15(2026-06-13)`COMPILER_FRONTEND_TOOLS` 增 `demo_feedback_log` → 前端 19、合计 58;**以 `len(ALL_TOOLS)` 实测为准**
...
- `Plugins/AgentBridge/Schemas/` — 58 工具的输入/输出 Schema 实体
