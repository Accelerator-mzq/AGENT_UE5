# Schemas Catalog — 41 主 Schema 字段级索引(+ 26 examples 附录)
...
> 数字注脚: spec v1.1 §3.2 写"64 Schema",实测主 catalog 41 + examples 26 = 67;实测为准。
...
## 附录 A: examples/ 示例数据(26 份)

`Plugins/AgentBridge/Schemas/examples/` 下是用于 Schema 验证 + 单元测试的 fixture 数据,**非契约本体**,不进主表。完整 26 份按文件名与对应 Schema 映射如下:
...
- 主 catalog 数据行数实测: **41**(已在主表逐行列出,与 `find ... -not -path "*/examples/*"` 一致)
- examples 附录数据行数实测: **26**(已在附录 A 逐行列出)
- 合计 **67**(spec v1.1 §3.2 写 64,实测 +3,以实测为准)
- 自检命令(Windows PowerShell 等价 / Git-Bash):
  - `find Plugins/AgentBridge/Schemas -name "*.json" -not -path "*/examples/*" | wc -l` → 41
  - `find Plugins/AgentBridge/Schemas/examples -name "*.json" | wc -l` → 26
