...
- **需要三路 Generator 完整验证**——LLM Internal（09A 产出）+ MCP Agent + Heuristic Fallback 各运行一次
- **需要更新证据报告**——覆盖三路 Generator 的产物、metadata 标记、promotable 判定
...
2. TASK 09B：Domain Skill Runtime 三路 Generator 完整验收（LLM Internal + MCP Agent + Heuristic Fallback）
