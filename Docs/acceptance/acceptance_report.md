...
- **测试**:Stage 13 SKS-01~94 登记(系统测试 270→364,含终审修复 +5:I-1 守卫绕过 1 + I-2 provenance 4),9 个 `test_phase13_*.py` 纯 Python。
...
## 附 4:Phase 14 Demo-First 增量主链(2026-06-12 新增)

### 附 4.1 交付摘要

Phase 14 实施 + 验收全部完成(分支 `feat/phase14-demo-first-spec`,实现 commit 范围 cd84dfe..739ecc1),交付:
...
- **Schema +2 + MCP 工具对**:`demo_plan` + `demo_story` schema + 2 examples 入 strict(28→30);MCP `demo_story_fetch` / `demo_story_submit`(`_make_response` 契约 + `plugin_root` 锚定),工具数 55→57。
- **CLI + 冒烟 runner**:`Scripts/demo_plan_main.py`(gap/run_id fail-closed + schema 自校验)+ `Scripts/demo_smoke/runner.py`(归因分离退出码 0/1/3)。
- **系统测试**:Stage 14 DMP-01~56 登记(6 个 `test_phase14_*.py` 纯 Python),系统测试 364→420(14 stage)。
- **项目层产物**:施工规范 `ProjectInputs/ConstructionManifest/demo_plugin_standards.md`(经 C4 PIVOT #1 升 1.1.0)+ 可玩 demo `Plugins/Demo_MonopolyAuction/`(authored 关卡 + 键盘可玩 + 拍卖增量,**项目层产物,不属 AgentBridge 框架**)。

权威数字(实测,2026-06-12):pytest `-k phase14` **56** / 系统测试 **420**(14 stage)/ Schema examples strict **30/30** / MCP 工具 **57**。

权威文档:spec [2026-06-11-phase14-demo-first-design.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/specs/2026-06-11-phase14-demo-first-design.md)(顶部含实施期修订记录)/ plan [2026-06-11-phase14-demo-first.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/plans/2026-06-11-phase14-demo-first.md)/ 任务书根 `task.md`。

### 附 4.2 验收状态(runbook 判据)

验收 runbook:[phase14_acceptance_runbook.md](/D:/UnrealProjects/Mvpv4TestCodex/ProjectState/Reports/2026-06-12/phase14_acceptance_runbook.md);PIVOT note:[phase14_v0_pivot_note_1.md](/D:/UnrealProjects/Mvpv4TestCodex/ProjectState/Reports/2026-06-12/phase14_v0_pivot_note_1.md)。

- **C1-C6 全闭环**,msc v1 终裁**通过**(2026-06-12):C1 机器全过(pytest 56 / Stage 14 / 全量等价 / strict 30,主会话亲跑);C2 切批标准答案 3 批 / 21 story / 拓扑序全中;C3 v0 无人值守经 1 次 PIVOT + 1 次试玩反馈修复轮后达成(17/17 verified,authored 关卡 + 键盘可玩 + 真实截图);C4 PROCEED(图形归 Phase 15);C5 增量批 1 拍卖 2/2 verified(hash 守门 + v0 回归实证);C6 真实 MCP stdio 实测 57 工具 fetch/submit 可见。
- **范围说明**:increment-2(股票市场)按 spec §2 留批不执行(Phase 14 范围 = v0 + 一次增量)。
- **已知预存失败**(非 Phase 14 引入,如实记录):MCP-03/04/05/08/10、P11-09/10/18、CP-44(gitignore 产物 / 历史证据缺失);SKS 4 条(Phase 13 验收 run 留树合成包,属 Phase 13 backlog,详见 runbook 附录 A)。

### 附 4.3 Phase 14 backlog(实施期审查累积)

- **P14-BL-01(安全)**:`evidence_validator` 路径型证据字段缺越界校验(`../` 可上跳项目外且 verified);应补 resolve + is_relative_to(同 MCP `plugin_root` 口径)。
- **P14-BL-02(机制)**:文档引用对账仅匹配反引号 + AUFE 前缀类名,裸写 / 非标前缀漏扫。
- **P14-BL-03(性能)**:`velocity.append_event` 全量读写 O(N²),Phase 15 扇出前改 JSONL。
- **P14-BL-04(可观测)**:smoke 报告 suites 丢 UE `errorMessage` 详情。
- **P14-BL-05(门禁缺口)**:宣称的交互(HUD/README 键位)无"必须有对应行为"的机器校验——C4 试玩暴露 Enter/Esc 空操作。
- **P14-BL-06(机制缺口)**:试玩反馈回流 story 无机制通道(修复轮发生在 story 机制外,证据与 story 状态漂移)。

### 附 4.4 Phase 15 输入(产品演进方向)

- **呈现 / 图形增量轴**(msc 试玩诉求):demo 当前只有文字 HUD,capability 切批机制缺呈现升级轴;Phase 15 须把"呈现/图形"作为增量分化维度。
- **词表扩展 / Stage 1 模板固化**(Phase 13 遗留)。
- ~~扇出 N demo + 挑选会话层~~:**经 msc 裁决(2026-06-12)自 Phase 15 范围去除**,恢复需 msc 主动提出;产品愿景"N 份 demo + 使用者挑选"保持为长期方向,不进近期阶段规划。


## 附 5:Phase 15 呈现增量轴 + 反馈回流通道(2026-06-13 新增)

### 附 5.1 交付摘要

Phase 15 机制层完成(分支 `feat/phase15-presentation-axis`),交付:

- **机制层(插件层)**:
  - planner amend()——切呈现/反馈批,读入 presentation_ladder.schema.json 定义的 rung 列表生成增量批
  - evidence_validator 行为校验门禁 BL-05——宣称的交互(HUD/键位)必须有对应行为机器校验,断言违例拒绝 story
  - 冻结分层 frozen_baselines / supersedes——baseline 版本固化 + 被取代链,防止旧 baseline 污染增量验证
  - MCP demo_feedback_log——试玩反馈回流登记工具,BL-06 落地,工具数 57→58
  - smoke runner BL-04 补全——suites 补 UE errorMessage 详情,归因分离退出码保持
  - Schema +2 入 strict(30→32)——presentation_ladder.schema.json + feedback_entry.schema.json 各 +1 example
  - demo_story / demo_plan schema 版本升 1.1.0(const 由 1.0.0 升 1.1.0,施工规范同步)
- **项目层(项目层产物)**:
  - 阶梯实例 `ProjectInputs/Ladder/monopoly_demo_ladder.json`(三 rung:文字 HUD→图形牌板→动画过渡)
  - 施工规范 `ProjectInputs/ConstructionManifest/demo_plugin_standards.md` 升 1.2.0
- **系统测试**:Stage 15 PRX-01~46 登记(纯 Python `test_phase15_*.py`),系统测试 420→466(15 stage)

权威数字(实测,2026-06-13):pytest `-k phase15` **46 passed** / `-k phase14` **56 passed** / Schema examples strict **32/32** / 系统测试 **466**(15 stage)/ MCP 工具 **58** / 红线零触碰。
...
权威文档:spec [2026-06-12-phase15-presentation-axis.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/specs/2026-06-12-phase15-presentation-axis.md) / plan [2026-06-12-phase15-presentation-axis.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/plans/2026-06-12-phase15-presentation-axis.md)

### 附 5.2 backlog 闭环

**Phase 15 机制层已落地 + 测试闭环的 backlog 条目**:

- **BL-01(路径越界)**:evidence_validator 路径型证据字段已补 resolve + is_relative_to 越界校验(同 MCP plugin_root 口径);PRX-01~05 覆盖越界拒绝断言。
- **BL-04(errorMessage)**:smoke runner suites 已补 UE errorMessage 详情,归因分离退出码 0/1/3 保持;PRX-06~10 覆盖 smoke 路径。
- **BL-05(行为校验门禁)**:evidence_validator 新增行为断言校验——宣称的交互必须有对应行为机器断言,C4 试玩暴露 Enter/Esc 空操作缺口已封堵;PRX-11~20 覆盖行为门禁路径。
- **BL-06(反馈回流)**:MCP demo_feedback_log 工具 + feedback_entry schema + 项目层 frozen_baselines 机制联合落地,试玩反馈有正式通道落盘;PRX-21~30 覆盖反馈回流路径。

**Phase 15 不纳入的 backlog 条目(msc 裁决)**:

- **BL-02(文档引用对账)**:裸写/非标前缀漏扫问题,msc 裁决不纳入 Phase 15(spec §5 明列),优先级低、收益边际。
- **BL-03(velocity O(N²))**:velocity.append_event 全量读写性能问题,扇出 N demo 经 msc 裁决去除后该 backlog 无痛点场景,spec §5 明列不进本阶段。
