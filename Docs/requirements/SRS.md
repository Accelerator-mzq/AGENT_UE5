...
> 关联 FEATURE_INVENTORY: Docs/FEATURE_INVENTORY.md(F-* ID 全量 105 行 · 15 family)
...
Mvpv4TestCodex 是一个以 AgentBridge 插件为通用 Agent 开发框架的 UE5 工程,采用"项目层(输入 / 配置 / 实例 / 治理)+ 插件层(通用编译 / 执行 / 验证机制)"双层结构。AgentBridge 不只是工具接口插件,而是包含 Compiler Plane(7 阶段 Skill-First 主链)、Reviewed Handoff 交接物机制、Orchestrator 执行编排、L1/L2/L3 受控工具体系、Run 治理与证据裁决的完整框架。本工程截至 2026-04-17 已完成 Phase 11 收尾,主链由 Root Skill Contract → Clarification Gate → Skill Graph Planning → Domain Skill Runtime → Cross Review v2 → Build IR v2 → Reviewed Handoff v3 构成,validate_examples.py --strict 26/26 通过,MCP 53 工具(49 正式 + 4 兼容 alias)已稳定运行,Monopoly 与 boardgame 端到端 run 已完成验收并落盘 ProjectState/runs/run-20260417-051425-aad0。
...
项目层全部(`ProjectInputs/` / `ProjectState/` / `Scripts/` / 根 `Source/` / 根 `task.md` 等) + 插件层全部(`Plugins/AgentBridge/` 下 Source / Scripts / Compiler / MCP / Schemas / SkillTemplates / Tests / AgentBridgeTests / Docs),共 15 个 F-* family、105 个 F-* ID。
...
L1 = 项目当前权威 Current/* + 插件层 Docs/* + 本 SRS 与新结构 contracts/;L2 = 已稳定的代码与 Schema 主体(`Plugins/AgentBridge/Source` / `Scripts/bridge` / `Compiler` / `MCP` / `Schemas/common`);L3-L5 = 已归档的 History/* 与 archive/*(仅追溯,不参与当前裁决)。Phase 11 已 Completed,Phase 12 入口待启,本 SRS 即为 Phase 12 之前的总基线。
...
- **数据契约**: → `contracts/mcp_tools_catalog.md`(53 工具字段级签名表) + `contracts/schemas_catalog.md`(compiler_session / evidence_manifest / run_comparison / batch_manifest)。
...
- **用途**: 承载 SkillGraph 数据结构、Domain Skill Runtime、Baseline 6 Skill 模板与类型扩展包(Genre Pack)。是 Compiler Stage 3-4 的语义骨架与模板基底。
- **对外接口**: `Plugins/AgentBridge/Compiler/skill_runtime/` + `Compiler/stages/domain_skill_runtime.py` + `Plugins/AgentBridge/SkillTemplates/baseline/` + `SkillTemplates/genre_packs/`。
- **功能列表**:
...
- F-CHN-03 Stage 3 — Skill Graph Planning — Gameplay + Baseline Domain Skills 规划,输出 `skill_graph.schema.json`。
...
- F-SCH-05 治理 Schema — `compiler_session` / `naming_resolution_log` / `design_decision_log` / `run_comparison` / `batch_manifest` / `evidence_manifest`。
- F-SCH-06 Examples 与版本清单 — `Schemas/examples/` + `Schemas/versions/` + 早期辅助(`gdd_projection` / `planner_output` / `run_plan`)。

完整清单(41 主 Schema + 26 examples = 67)详见 `contracts/schemas_catalog.md`。

### 5.2 MCP 工具体系

53 个 MCP 工具(49 正式 + 4 兼容 alias)的字段级签名表见 `contracts/mcp_tools_catalog.md`;权威源是 `Plugins/AgentBridge/MCP/tool_definitions.py` 的 `len(ALL_TOOLS) == 53` 实测。
...
- Schema 校验:`Plugins/AgentBridge/Scripts/validation/validate_examples.py --strict`,当前 `26/26` 通过;任一示例不符 → 退出码非 0,阻断 commit。
- 系统测试:`Plugins/AgentBridge/Tests/run_system_tests.py`,登记 240 条系统测试 + L1/L2/L3 共约 26 条 C++ 测试 = 266 条总测试用例。
...
MCP Server(F-MCP-13)走 stdio 通道,53 个工具按命名空间组织(bridge_l1_* / bridge_l2_* / compiler_prepare_* / compiler_save_* / evidence_* / run_* 等);兼容 alias(F-MCP-12)4 个保留旧名映射。
