...
> 版本: v1.3(2026-06-13,Phase 15 权威数字同步);v1.2(2026-06-12,Phase 14 权威数字同步)
...
5. **测试与验收**:
   - `Docs/testing/test_spec.md` — 466 系统测试用例索引
   - `Docs/acceptance/acceptance_report.md` — Phase 11 基线 + 回归门禁 + Phase 13/14 附录
...
│   ├── schemas_catalog.md                    # 49 主 Schema + 32 examples
│   └── mcp_tools_catalog.md                  # 58 MCP 工具签名表
├── superpowers/
...
| 硬事实 | 权威文档 | 数字 / 内容 |
|--------|----------|-------------|
| MCP 工具数 | `Docs/contracts/mcp_tools_catalog.md` 主表 | **58**(Bridge 28 + 前端 19 + 后端 11,demo 工具对注册在 `COMPILER_FRONTEND_TOOLS` 内;2026-06-11 Phase 13 +`compiler_skill_synthesis_prepare/save`=55;2026-06-12 Phase 14 +`demo_story_fetch/submit`=57;2026-06-13 Phase 15 +`demo_feedback_log`=58,实测 `tool_definitions.ALL_TOOLS`)|
| Schema 数 | `Docs/contracts/schemas_catalog.md` 主表 + 附录 A | **49 主 + 32 examples = 81**(2026-06-13 Phase 15 +`presentation_ladder`/`feedback_entry` schema 与 example 各 2;2026-06-12 Phase 14 +`demo_plan`/`demo_story` schema 与 example 各 2;2026-06-11 Phase 13 新增 `gdd_coverage_matrix` schema + 1 example,同日补登记 Phase 12 漏录的 `provider_call`/`retry_policy`;2026-05-27 ForgeUE +`forgeue_import_evidence`)|
| 系统测试用例数 | `Docs/testing/test_spec.md` §3 + `Plugins/AgentBridge/Tests/run_system_tests.py` TOTAL_CASES | **466**(15 stage;2026-06-11 Phase 13 +Stage 13 SKS=364;2026-06-12 Phase 14 +Stage 14 DMP-01~56=420;2026-06-13 Phase 15 +Stage 15 PRX-01~46=466,实测 `run_system_tests.py` TOTAL_CASES)|
...
| Schema --strict 32/32 | `Docs/testing/test_spec.md §5` + `Docs/acceptance/acceptance_report.md §2.1` | `validate_examples.py --strict`(2026-06-11 27→28 加 `phase13_gdd_coverage_matrix`;2026-06-12 Phase 14 +`phase14_demo_plan`/`phase14_demo_story` example=30;2026-06-13 Phase 15 +`phase15_presentation_ladder`/`phase15_feedback_entry` example=32,实测 Checked/Passed 32)|
