...
- **MCP Server (stdio)**:`MCP/server.py`,挂在 Agent IDE / CLI 一端,通过 stdio 协议暴露 55 工具(28 Bridge Passthrough + 16 前端 Compiler + 11 后端 Evidence,F-MCP-13;Phase 13 增合成工具对)。MCP Server 不是流程总线,而是认知桥接层。
...
| Stage 3 | Skill Graph Planning | Stage 1 + Stage 2 | `skill_graph.schema.json`(Phase 13 起含 `metadata.capability_gaps`) | F-CHN-03 |
| S3.5(条件) | Skill Synthesis(Phase 13) | Stage 3 `capability_gaps` + GDD 上下文 | `SkillTemplates/synthesized/<capability_id>/` 6 文件包 + `synthesis_review.md` | F-CMP-25 |
...
边界规则:Stage 1 通过 `compiler_root_skill_prepare/save`(F-MCP-06)、Stage 2 通过 `compiler_clarification_prepare/save`(F-MCP-07)、Stage 3 通过 `compiler_skill_graph_prepare/save`(F-MCP-08)暴露给 Agent,Agent 直接驱动认知分解;S3.5(Phase 13 条件环节)通过 `compiler_skill_synthesis_prepare/save` 暴露,`capability_gaps` 非空且 `allow_skill_synthesis=true` 时由 Agent 现场合成缺件 SkillTemplate(机器校验 + 人审 `review_status: approved` 双 gate,harness 中立,Claude Code / Codex 双端可驱动);Stage 4-7 由 Compiler Core 内部调度,Agent 仅在 Stage 4 通过 `compiler_stage4_node_prepare/save`(F-MCP-09)参与每个 Skill Instance 的 Discovery / Candidates / Convergence 三个 phase,Stage 5-7(Cross Review / Lowering / Handoff)对外不开放交互接口。Stage 之间不允许跳序,前一阶段产物未落盘前后一阶段拒绝执行(由 `compiler_session.schema.json` 的 `current_stage` + `stage_outputs` 字段守护)。详细字段映射见 `Docs/contracts/schemas_catalog.md` 与 `Docs/contracts/mcp_tools_catalog.md`。
...
**治理规则**(F-GOV-02 / F-GOV-03 / F-GOV-05):
...
- (Phase 13)run 消费了 synthesized skill(skill_graph 节点 `template_source=synthesized`)→ `PROMOTE_REJECTED`(试制章,与 heuristic_fallback 同级保守;转正机制记 backlog)。
- (Phase 13)run 存在未解决 `capability_gaps`(合成关闭 / 未完成时 gap 保留)→ `PROMOTE_REJECTED`(产物不完整);skill_graph 损坏 / 不可解析时 fail-closed 同样拒绝。
...
**MCP Server 主入口**(F-MCP-13)通过 stdio 暴露 55 工具:Bridge L1 查询 7 / L1 写 6 / L1 服务 5 / L2 资产 9 / L3 兜底 1(共 28 = F-MCP-01..05 的 Bridge Passthrough)+ 前端 Compiler 16(F-MCP-06..09 + F-MCP-12 alias + Phase 13 合成工具对 `compiler_skill_synthesis_prepare/save`)+ 后端 Evidence 11(F-MCP-10 / F-MCP-11)。Bridge Passthrough 28 工具在新口径下定位为"可选外部协议适配层",非 MCP 核心价值;MCP 核心价值在前端 Stage 1-3 + S3.5 认知分解 / 合成与后端证据裁决。

> **通道数实测注脚**:契约层 4 通道是逻辑划分(`tool_contract.md` §5.2)。代码层 `BridgeChannel` 枚举定义 **4 个值**(`PYTHON_EDITOR` / `REMOTE_CONTROL` / `CPP_PLUGIN` / `MOCK`,`bridge_core.py:44-62`),但 MCP Server 的 `channel_map`(`server.py:55-71`)只暴露 `MOCK` / `REMOTE_CONTROL` / `CPP_PLUGIN` 三档供 `AGENTBRIDGE_MCP_CHANNEL` env 选择,默认 `CPP_PLUGIN`。`PYTHON_EDITOR` 通道在 UE 5.5.4 下因 `EditorLevelLibrary.GetEditorWorld` 不能稳定通过 RC 远程调用而默认不走(`server.py:69-71`)。HLD 沿用 4 通道契约划分保持语义可分性,代码层运行时常用 3 通道。

→ §4 工具签名与错误码族详见 `Docs/contracts/tool_contract.md` §2-§6;55 工具的字段级输入/输出 schema 映射详见 `Docs/contracts/mcp_tools_catalog.md`;字段命名与单位规范详见 `Docs/contracts/field_specification.md`。详细 Bridge / MCP 实现切分见 LLD `Docs/design/LLD/02_bridge.md` 与 `Docs/design/LLD/05_mcp.md`(占位)。
...
**共享层与治理层**:`common/` 6 份(bounds / collision / error / material / primitives / transform,F-SCH-03);`feedback/` 5 子目录(actor / asset / level / project / validation)+ `write_feedback/` 1 份(F-SCH-04);治理 9 份(`compiler_session` / `naming_resolution_log` / `design_decision_log` / `run_comparison` / `batch_manifest` / `evidence_manifest` + Phase 12 `provider_call` / `retry_policy` + Phase 13 `gdd_coverage_matrix`,F-SCH-05);examples 28 份(F-SCH-06)。主 catalog 45 + examples 28 = 73(实测,详见 `Docs/contracts/schemas_catalog.md` 附录 B)。
...
测试体系按"语义层 + 执行后端"两维划分,共 17 类 364 条用例(`Plugins/AgentBridge/Tests/SystemTestCases.md` §1)。
...
**系统测试套件**(F-TST-04):`run_system_tests.py` 注册 13 个 Stage 共 364 条用例,分 17 类 (SV/BL/Q/W/CL/UI/CMD/PY/ORC/CP/SS/GA/E2E/MCP/P11/LIR/SKS)。证据落盘到 `ProjectState/Reports/YYYY-MM-DD/` + `ProjectState/Evidence/`,历史归档复制到 `Docs/History/reports/AgentBridgeEvidence/`(`Docs/requirements/SRS.md#6.3`)。
...
- **ADR-001 文档治理结构**(`Docs/archive/decisions/ADR-001-Doc-Governance.md`):采用五层文档治理(L0 入口 / L1 当前生效 / L2 插件 Canonical / L3 历史归档 / L4 决策 + L5 草案),task.md 定位为一次性消耗品,AI 默认只读 L0+L1+L2。本次 docs-restructure-for-ue57 重构在 ADR-001 框架内做实现(将散落规则合并为 `Docs/governance.md` + `Docs/contracts/*` + `Docs/design/*` + `Docs/requirements/*`)。
- **ADR-002 Task 与 Evidence 归档规则**(`Docs/archive/decisions/ADR-002-Task-And-Evidence-Archiving.md`):根目录 task.md 是当前阶段唯一任务入口,阶段结束后归档到 `Docs/History/Tasks/taskN_phaseX.md`;Snapshots / Evidence / Reports 分目录;系统测试用例阶段结算后再补录到 `SystemTestCases.md`。
...
| `Docs/contracts/mcp_tools_catalog.md` | §2 / §4 MCP 55 工具签名 |
...
| `Docs/archive/decisions/ADR-001-Doc-Governance.md` | §8 ADR 引用 |
| `Docs/archive/decisions/ADR-002-Task-And-Evidence-Archiving.md` | §8 ADR 引用 |
