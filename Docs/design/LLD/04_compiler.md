...
> 版本: v1.1 (2026-06-11,Phase 13 S3.5 合成链同步)
> 范围: Legacy v1 Pipeline(`Plugins/AgentBridge/Scripts/compiler/`,6 子目录)+ Phase 11 Compiler Framework(`Plugins/AgentBridge/Compiler/`,pipeline + 12 stages + Phase 12 LLM Provider 子树 + Phase 13 S3.5 合成链 4 模块 + v1 framework 残留)共 ~35 个 Python 模块
...
| S3.5 注册表扫描(Phase 13) | `Plugins/AgentBridge/Compiler/stages/registry_scan.py` | F-CMP-25 | 扫描 manifest `capability_bindings` + 占位数据 + synthesized(仅 approved)建 capability→template 映射 + family 白名单 |
| S3.5 合成校验器(Phase 13) | `Plugins/AgentBridge/Compiler/stages/synthesis_validator.py` | F-CMP-25 | 双 gate 第一关:6 文件结构/manifest 必填/capability_id 格式硬校验/递归 additionalProperties/family 白名单 |
| S3.5 Skill 合成(Phase 13) | `Plugins/AgentBridge/Compiler/stages/skill_synthesis.py` | F-CMP-25 | prepare 载荷组装(gap 上下文+规范+范例)+ save 事务落盘(.part)+ 审阅清单生成 |
| GDD 覆盖矩阵(Phase 13) | `Plugins/AgentBridge/Compiler/stages/gdd_coverage.py` | F-CMP-26 | markdown 结构切分(零语义)+ anchor 认领 → 覆盖矩阵 sidecar(claimed/unclaimed/container 三态) |

12 Stage 模块按职责分四类:**契约层** Stage 1/2(F-CMP-08/09)生成根契约与澄清门;**规划层** Stage 3(F-CMP-10)生成 Skill Graph;**领域层** Stage 4(F-CMP-19)三 phase 编排 + 三 fallback(F-CMP-11/12/13);**收尾层** Stage 5/6/7(F-CMP-17/18/16)审查 + 下降 + 组装。Stage 通用支撑(F-CMP-14/15)横切所有 Stage。Phase 13 在规划层与领域层之间插入条件环节 **S3.5 Skill Synthesis**(F-CMP-25/26,见 §2.1.2),`capability_gaps` 为空时零影响。
...
### 2.1.2 Phase 13 S3.5 Skill 合成链(2026-06-11 实施完成)

Phase 13 解决"Skill 封闭世界"缺口:Stage 3 原 `GAMEPLAY_NODE_CONFIGS` / `BASELINE_NODE_CONFIGS` / `FRAGMENT_FAMILY_MAP` 三张硬编码表全删,注册信息下沉为模板 `manifest.yaml` 的 `capability_bindings` 块 + `SkillTemplates/registry_placeholders.yaml` 占位数据;库外 required capability 不再静默丢弃,写入 `skill_graph.metadata.capability_gaps[]`。四个新模块(合成链按惯例彼此 importlib 动态加载,便于独立测试):

- `Compiler/stages/registry_scan.py`(221 行)— `scan_capability_registry()` 三遍扫描(正式库 → 占位 → synthesized approved,synthesized 不得压过正式库)+ `execution_family_whitelist()` 数据驱动白名单;
- `Compiler/stages/synthesis_validator.py`(319 行)— `validate_synthesized_package()` / `validate_capability_id()`:6 文件齐全、manifest 必填、`template_id` 与目录一致、capability_id 格式硬校验(堵路径穿越)、output_schema 合法 draft-07 且 root + 嵌套 object(含 `oneOf/anyOf/allOf` 组合器分支,带递归深度护栏)全部 `additionalProperties: false`、`can_emit_families` ⊆ 白名单;返回错误列表供 agent 修正重提;
- `Compiler/stages/skill_synthesis.py`(297 行)— `build_synthesis_prepare_payload()`(gap 上下文 + 6 文件规范 + 最相近 1-2 个现有模板 few-shot + 白名单 + 命名规则)/ `save_synthesized_package()`(先校验后落盘,`.part` 临时目录事务写入,approved 包防覆盖,`six_files` 出现规范外 key 一律 rejected)/ `generate_synthesis_review()`(人审清单 `ProjectState/runs/{run_id}/synthesis_review.md`);
- `Compiler/stages/gdd_coverage.py`(140 行)— `split_gdd_sections()`(只认 markdown 结构,零游戏语义)/ `build_coverage_matrix()`(anchor 字符串匹配认领,`claimed` / `unclaimed` / `container` 三态)/ `render_coverage_markdown()`(渲染省略可审计);防固化四守则(切分器最笨 / 裁决 run 级 / 只靠 anchor / 格式假设显式)写入模块 docstring,实施与评审强制对照。

MCP 暴露面:`compiler_skill_synthesis_prepare/save` 工具对(tool_definitions / server / compiler_tools 三处注册,工具数 53→55)。治理面:`allow_skill_synthesis` session 开关(默认 false,fast_mode 下无条件跳过)、开启时 Stage 1 required capability 强制 `source_anchor` 留痕(intake alias 转发同样受控)、synthesized 消费与未解决 gap 的 promote 双守卫(见 §4.3)。

完整 spec:[Docs/superpowers/specs/2026-06-10-phase13-skill-synthesis-design.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/specs/2026-06-10-phase13-skill-synthesis-design.md)(顶部含实施期修订记录)。
完整 plan:[Docs/superpowers/plans/2026-06-10-phase13-skill-synthesis.md](/D:/UnrealProjects/Mvpv4TestCodex/Docs/superpowers/plans/2026-06-10-phase13-skill-synthesis.md)。
验收 runbook:[ProjectState/Reports/2026-06-11/phase13_acceptance_runbook.md](/D:/UnrealProjects/Mvpv4TestCodex/ProjectState/Reports/2026-06-11/phase13_acceptance_runbook.md)(判据 1-4 闭环,5-12 待执行)。
...
def create_skill_graph(root_skill_contract, clarification_gate_report, run_id=None) -> Dict[str, Any]  # 构造 SkillGraph 节点 + 边;库外能力记 metadata.capability_gaps
def save_skill_graph(skill_graph: Dict[str, Any], output_path: str | Path) -> str  # 落盘

# S3.5 合成链(Phase 13,F-CMP-25/26;模块间 importlib 动态加载)
# registry_scan.py
def scan_capability_registry(templates_root=None) -> Dict[str, Dict[str, Any]]  # capability→template 映射(三遍扫描)
def execution_family_whitelist(templates_root=None) -> set[str]  # 执行层 family 白名单(数据驱动)
# synthesis_validator.py
def validate_synthesized_package(capability_id, package, family_whitelist) -> List[str]  # 合成包结构校验,返回错误列表
def validate_capability_id(capability_id) -> List[str]  # capability_id 格式硬校验(堵路径穿越)
# skill_synthesis.py
def build_synthesis_prepare_payload(...) -> Dict[str, Any]  # 合成 prepare 载荷(上下文+规范+范例+白名单)
def save_synthesized_package(...) -> Dict[str, Any]  # 校验+.part 事务落盘+approved 防覆盖
def generate_synthesis_review(...) -> str  # 人审清单 synthesis_review.md
# gdd_coverage.py
def split_gdd_sections(gdd_text: str) -> List[Dict[str, Any]]  # markdown 结构切分(零语义)
def build_coverage_matrix(...) -> Dict[str, Any]  # anchor 认领,claimed/unclaimed/container 三态
def render_coverage_markdown(matrix: Dict[str, Any]) -> str  # 人读版渲染(省略可审计)
...
GDD (.md)
  ...
  └─ pipeline_orchestrator.run_pipeline(session, range(1,8))
     ...
     │           └─ metadata.capability_gaps[](Phase 13:库外能力显式记录)
     ├─ S3.5: Skill Synthesis(Phase 13 条件环节,capability_gaps 非空才介入)
     │           ├─ allow_skill_synthesis=false(默认)→ 跳过合成,gap 保留,
     │           │   run 不可 promote(§4.3 守卫;默认行为与 Phase 11 一致)
     │           └─ =true → 逐 gap 走 MCP compiler_skill_synthesis_prepare/save
     │               → 机器校验(synthesis_validator)→ 人审 gate(review_status=approved)
     │               → 重跑 Stage 3(新模板被扫描到,gap 清空)
...
evidence 层 promote 守卫(Phase 13,F-GOV-05,MCP/evidence_tools.py):
  run 含 template_source == "synthesized" 节点      → PROMOTE_REJECTED(试制章)
  run 的 skill_graph.metadata.capability_gaps 非空   → PROMOTE_REJECTED(产物不完整)
  skill_graph 损坏 / 不可解析                        → fail-closed,同样拒绝
```

`fast_mode` 在 Stage 2 触发跳过部分澄清项;`generator_provider` 在 Stage 4 任一节点走 heuristic 时被 pipeline_orchestrator 自动写入(`if gp_type and not session.generator_provider: session.generator_provider = gp_type`)。**这两个 flag 是一旦置位不可清零的单向降级**。Phase 13 的 synthesized/gap 守卫落在 evidence 层(`evidence_promote_run` 拒绝路径,与 heuristic_fallback 同级保守),synthesized 标记只认 skill_graph 节点的 `template_source` 单一事实源。
...
- **新增 Domain Skill Template**:见 `LLD/06_skills_and_templates.md` §5,在 `Plugins/AgentBridge/SkillTemplates/` 加 6 文件并在 `manifest.yaml` 写 `capability_bindings` 块,Stage 3 经 `registry_scan` 扫描自动拾取(Phase 13 起注册表数据化),**Compiler 侧不需要改代码**;GDD 编译期缺件也可走 S3.5 合成路径(MCP 工具对 + 双 gate)。
