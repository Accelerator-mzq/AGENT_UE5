...
> 版本: v1.1 (2026-06-11,Phase 13 注册表数据化同步)
> 范围: Skill Runtime(`Plugins/AgentBridge/Compiler/stages/skill_graph_planning.py` 635 行 + `domain_skill_runtime.py` 1829 行 + Phase 13 新增 `registry_scan.py` 221 行 + v1 残留 `Compiler/skill_runtime/skill_runtime.py` 100 行 / `Compiler/planner/planner.py` 96 行)+ SkillTemplate 静态模板(`Plugins/AgentBridge/SkillTemplates/` 下 12 skill × 6 文件 = 72 文件 + `registry_placeholders.yaml` 占位数据 + `synthesized/` 合成隔离区)
...
|---|---|---|---|
| Stage 3 Skill Graph Planning(v2 主链) | 读 Root Skill Contract + Clarification Gate Report,生成 Gameplay/Baseline 节点 + 依赖边 + 收敛序号 → skill_graph.json;Phase 13 起节点配置走注册表扫描,库外能力记 `metadata.capability_gaps` | F-SKL-01 | `Plugins/AgentBridge/Compiler/stages/skill_graph_planning.py`:635 |
| 注册表扫描(Phase 13 新增) | 扫描全部 manifest `capability_bindings` + `registry_placeholders.yaml` + `synthesized/`(仅 `review_status: approved`)构建 capability→template 映射 + 执行 family 白名单 | F-CMP-25 | `Plugins/AgentBridge/Compiler/stages/registry_scan.py`:221 |
| Stage 4 Domain Skill Runtime(v2 主链) | 按拓扑顺序运行每个 Node 的 Discovery / Candidates / Convergence 三 phase,聚合 Stage 4 三份总览 + skill_fragments/* | F-SKL-02 | `Plugins/AgentBridge/Compiler/stages/domain_skill_runtime.py`:1829 |
...
Phase 13 起模板层另有三类配套数据(不计入 12×6 网格):

- **manifest `capability_bindings` 块**:每个模板 manifest 自描述其承接的 capability_id、`allows_design_space_discovery`、`convergence_priority`、fragment family 等(原 Stage 3 硬编码表逐字下沉),Stage 3 经 `registry_scan` 扫描消费;
- **`SkillTemplates/registry_placeholders.yaml`**:无模板占位节点数据(input / audio / platform foundation 等 `template_source: future_baseline_template` 条目),保证等价回归不破;
- **`SkillTemplates/synthesized/<capability_id>/`(合成隔离区)**:S3.5 由 agent 现场合成的 6 文件包落盘处,manifest 带 `template_source: synthesized` + `review_status`(初始 `pending_review`);**仅 `review_status: approved` 的会被 `registry_scan` 纳入映射**,未审包编译器不可见。消费了 synthesized 模板的 run 一律 `promotable=false`(试制章,见 HLD §3)。
...
def _build_gameplay_nodes(root_skill_contract, clarification_gate_report) -> List[Dict[str, Any]]:
    # 按 registry_scan 注册表映射按 capability.activation == "required" 过滤生成节点;
    # 查不到映射的 required capability 记入 metadata.capability_gaps(Phase 13 起零静默丢弃)
...
# 注:原 GAMEPLAY_NODE_CONFIGS / BASELINE_NODE_CONFIGS 两张硬编码表已于 Phase 13 删除,
# 节点配置改由 registry_scan.scan_capability_registry() 扫描 manifest capability_bindings
# + registry_placeholders.yaml 构建(_load_registry_scan 惰性加载)
```

**Phase 13 — `Plugins/AgentBridge/Compiler/stages/registry_scan.py`(221 行):**

```python
def scan_capability_registry(templates_root: str | Path | None = None) -> Dict[str, Dict[str, Any]]:
    # 三遍扫描(正式库 → 占位数据 → synthesized approved)构建 capability_id → 节点配置映射;
    # synthesized 不得压过正式库(优先级修复见 commit 6643c09);必填字段缺失告警跳过
def execution_family_whitelist(templates_root: str | Path | None = None) -> set[str]:
    # 从注册表派生执行层 family 白名单(合成校验器消费,数据驱动)
```
...
# 全局常量
FRAGMENT_FAMILY_MAP   # skill-* → *_spec family;Phase 13 起由 _build_fragment_family_map() 从注册表
                      # can_emit_families 惰性派生(原 16 项硬编码表已删),长驻进程可见运行期 approve 的模板
...
capability_bindings:                                  # Phase 13 新增:模板自描述的能力绑定块,列表项字段为
                                                      # capability_id / instance_id / convergence_priority /
                                                      # related_clarification_items / planning_notes / fragment_family
                                                      # (原 Stage 3 硬编码表逐字下沉;synthesized 模板另带
                                                      # template_source: synthesized + review_status,approved 才被扫描纳入)
```

## 4. 数据流与状态机

**场景 A — Skill Graph 生成(Stage 3,F-SKL-01):** Pipeline Orchestrator 把上游 `root_skill_contract.json` + `clarification_gate_report.json` 喂入 `create_skill_graph()`;`_build_gameplay_nodes` 按 `registry_scan.scan_capability_registry()` 构建的 capability→template 映射(Monopoly 基线下与原 7 candidate 等价:`skill-board-topology` / `-tile-system` / `-turn-loop` / `-dice` / `-economy` / `-player-management` / `-jail`,由 golden 等价回归守门)按 `capability.activation == "required"` 过滤生成 gameplay 节点,每个节点的 `template_id` 指向对应 SkillTemplate;**查不到映射的 required capability 写入 `metadata.capability_gaps[]`(capability_id / 来源 anchor / 原因),不再静默丢弃**;`_build_baseline_nodes` 读 Root Skill Contract 的 `baseline_capabilities` 对每个 baseline 域生成节点,`_resolve_baseline_template_source` 检查 `SkillTemplates/baseline/<domain>/` 6 文件是否齐全,缺即标 `missing`(进入 `metadata.missing_baseline_templates` 警告列表)。`_filter_edges` 从静态 edge 表(`EDGE_BLUEPRINTS`,Phase 13 保留在代码)过滤出当前节点集合内部的边,`_build_relationship_maps` 派生 `dependency_map` / `coupling_map`,最终 `_annotate_node` 在每个节点上追加 `dependencies` + `coupling`(按 order_map 排序);`allows_design_space_discovery` 和 `convergence_priority` 由注册表 binding 数据驱动、在各自 `_build_*_nodes` 设置;落 `skill_graph.json`。
...
2. **新增 Genre Pack(全新游戏类型):** 在 `Plugins/AgentBridge/SkillTemplates/genre_packs/<genre>/<variant>/` 建目录树(如 `genre_packs/rpg/jrpg/`),为该 Genre 下每个 skill 建 6 文件子目录,并在各自 `manifest.yaml` 写 `capability_bindings` 块(capability_id / allows_design_space_discovery / convergence_priority / fragment family)。Phase 13 起注册表由 manifest 自描述,**无需改任何代码**——`registry_scan` 启动扫描自动纳入,fragment family 同步从 `can_emit_families` 派生。当前 inventory 中 `jrpg_genre_pack_design.md` / `genre_pack_core_design.md` 即为新 Genre 设计的输入参考。另一条轻量路径:GDD 编译时缺件由 S3.5 合成环节(`compiler_skill_synthesis_prepare/save`)现场合成进 `synthesized/` 隔离区,人审 approved 后即可被消费(试制件,run 不可 promote;转正机制记 backlog)。
