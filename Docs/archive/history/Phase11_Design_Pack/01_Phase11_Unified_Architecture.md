...
- Phase 11：`53` 个可见注册工具
  - `49` 个正式主工具
  - `4` 个兼容 alias（`compiler_intake_prepare/save`、`compiler_plan_prepare/save`）

说明：实施前文档中的“50”属于 planning 口径。收尾时以 `tool_definitions.ALL_TOOLS` 与 `server.TOOL_DISPATCH` 的实际注册结果为准。
