...
> 阶段证据归档：[phase6_evidence_2026-04-02](/D:/UnrealProjects/Mvpv4TestCodex/Docs/History/reports/AgentBridgeEvidence/phase6_evidence_2026-04-02)
...
- `E2E-28`：如果目标是棋类 / boardgame，则必须包含一张 `topdown_alignment` 顶视图，且该图必须完整覆盖棋盘边界与全部棋子，并能直接判断棋子和格子 / 棋盘坐标的对应关系

【证据要求】
- `ProjectState/Evidence/Phase6/screenshots/`
- `ProjectState/Evidence/Phase6/logs/`
- `ProjectState/Evidence/Phase6/notes/`
- 对棋类 / boardgame，截图证据至少包含：
  - `overview_oblique`
  - `topdown_alignment`
...
- `E2E-28`：棋类 / boardgame 顶视图证据完整性验证通过
...
- `E2E-27`：真实截图证据写入 `ProjectState/Evidence/Phase6/`，且对棋类 / boardgame 必须包含能完整看到棋盘和全部棋子的 `topdown_alignment` 顶视图
- `E2E-28`：棋类 / boardgame 的 `topdown_alignment` 顶视图完整覆盖棋盘边界与全部棋子，且能直接判断棋子和格子 / 棋盘坐标的对应关系
