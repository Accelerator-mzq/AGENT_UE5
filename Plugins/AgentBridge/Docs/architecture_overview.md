...
> 目标引擎版本：UE5.5.4 | 文档版本：v0.9.0 | 适用范围：AgentBridge 通用 Agent 开发框架
...
### 1.1 总链路（v0.9.0 — Phase 10 MCP 重定位）

```
GDD (Part A + Part B)
→ MCP 前端认知分解层（Stage 1-2）
  ├── GDD 语义理解 → Root Skill 形成
  └── Sub-Skill Graph 分解 → Spec Family 映射建议
→ Skill-First Compiler（6 阶段主链）
  ...
  注：Stage 3-5 由 Compiler Core 调度，Agent 通过 Core 内部 prepare/save 参与
...
- 后端：负责测试证据判读，输出 pass / fail / escalate
...
```
 ...
 │                                                                     │
 │  ┌───────────────────────────────────────────────────────────────┐  │
 │  │         MCP 前端：认知分解接口（Stage 1-2）                    │  │
 │  │                                                               │  │
 │  │  compiler_intake_prepare/save  — Stage 1: GDD → Projection   │  │
 │  │  compiler_plan_prepare/save    — Stage 2: → Planner Output   │  │
 │  │                                                               │  │
 │  │  AI Agent 通过 MCP 工具驱动认知分解，完成后交付 Compiler Core  │  │
 │  └──────────────────────────┬────────────────────────────────────┘  │
 │                             │ GDD Projection + Planner Output       │
 │  ═══════════════ MCP 前端边界｜Stage 3-5 由 Compiler Core 调度 ═══  │
 │                             ▼                                       │
 │  ┌───────────────────────────────────────────────────────────────┐  │
 │  │           Skill Compiler Plane — Compiler Core                │  │
 │  │                                                               │  │
 │  │  Stage 3: Skill Runtime — Template Pack → Skill Fragments    │  │
 │  │    （结构受 Static Spec 约束，内容由 AI Agent 填充）           │  │
 │  │  Stage 4: Cross-Review — Fragments → Review Report           │  │
 │  │  Stage 5: Lowering — Reviewed Spec Tree → Build IR           │  │
 │  │                                                               │  │
 │  │  Agent 通过 Core 内部 prepare/save 参与，不通过 MCP 对外工具  │  │
 ...
 └─────────────────────────────┼───────────────────────────────────────┘
    ...
    └──────────────────────────┬───────────────────────────────────┘
                               │ 测试证据
    ┌──────────────────────────▼───────────────────────────────────┐
    │         MCP 后端：证据裁决接口（Evidence Judge）              │
    │                                                              │
    │  读取测试证据 → Agent 判读 → pass / fail / escalate          │
    │  不直接控制 PIE、不模拟输入——仅消费已有证据做裁决            │
    │  人工确认门仍保留                                            │
    └─────────────────────────────────────────────────────────────┘
```

> **MCP 定位说明**：MCP 从执行层 Bridge 包装重新定位为认知桥接层。
> 前端（Stage 1-2）负责 GDD → Projection → Planner Output 的认知分解；
> 中段 Bridge Passthrough 28 工具保留为可选外部协议适配层；
> 后端负责测试证据判读（pass / fail / escalate）。
...
│                                                                 │
│    ┌─ MCP 前端认知分解层（Stage 1-2）───────────────────────┐    │
│    │  GDD 语义理解 → Root Skill → Sub-Skill Graph          │    │
│    │  → Spec Family 映射建议 → 冲突/缺口/风险提示          │    │
│    │  止于此。不控制 Stage 3-5。                            │    │
│    └───────────────────────────┬────────────────────────────┘    │
│                                ▼                                │
│    ┌─ Skill Compiler Plane（编译前端）─────────────────────┐    │
│    │  Compiler Core 调度 Stage 3-5：                        │    │
│    │  Skill Runtime → Cross-Review → Lowering → Handoff    │    │
│    │  Agent 通过 Core 内部 prepare/save 参与                │    │
│    │  输入：GDD / Presets / Project State                  │    │
│    │  输出：Reviewed Handoff v2                             │    │
│    └───────────────────────────┬────────────────────────────┘    │
│                                ▼                                │
│    ┌─ Execution Orchestrator Plane（执行编排）──────────────┐    │
...
| `Docs/Current/14_MCP_Cognitive_Bridge_Anchor.md` | MCP 认知桥接层总口径声明（优先裁决依据） |
