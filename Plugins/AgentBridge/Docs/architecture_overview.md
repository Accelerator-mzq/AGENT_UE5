...
> 目标引擎版本：UE5.5.4 | 文档版本：v1.0.0 | 适用范围：AgentBridge 通用 Agent 开发框架
...
### 1.1 总链路（v1.0.0 — Phase 11 Skill-First Design Compiler）

```
GDD
→ MCP 前端认知桥接层（Stage 1-3）
  ├── Stage 1: Root Skill Contract 形成（含 Universal Baseline Merge）
  ├── Stage 2: Clarification Gate（4 档决策 + provisional 传播 + fast_mode 高风险保护）
  └── Stage 3: Skill Graph Planning（Gameplay + Baseline Domain Skills 规划）
→ Compiler Core（Stage 4-7）
  ├── Stage 4: Domain Skill Runtime — 三路生成策略
  │   ├── MCP Agent（主路径：prepare/save 交互，Agent 本身即 LLM）
  │   ├── LLM Internal（pipeline 内嵌 LLM API 调用）
  │   └── Heuristic Fallback（确定性退化，CI/fast_mode）
  │   含: Design Space Discovery → Realization Candidates → Convergence → Fragment Generation
  ├── Stage 5: Cross-Domain Convergence Review v2
  ├── Stage 6: Lowering → Build IR v2（含 naming_resolution_log + Baseline 步骤）
  └── Stage 7: Handoff Assembly → Reviewed Handoff v3
→ Reviewed Handoff v3（Compiler → Execution 唯一边界）
→ Execution Orchestrator Plane（执行编排）
  ├── Build Steps → C++ 代码生成
  ├── Bridge Passthrough（MCP 28 工具，可选外部协议适配层）
  └── Validation IR → 验证检查点
→ UE 官方自动化测试主干
  └── Automation Test / Spec / Functional / Driver / Gauntlet / UAT
→ MCP 后端证据裁决层
  └── 证据读取 → 证据判读 → pass / fail / escalate
  └── Run 治理 → compare / promote / batch
→ Playable Template
```

**MCP 定位说明**：MCP 在本框架中的角色是**认知桥接层**（Cognitive Bridge Layer），不是执行主干或测试执行总线。
- 前端（Stage 1-3）：负责 GDD → Root Skill Contract → Clarification Gate → Skill Graph Planning 的认知分解
- Stage 4-7 由 Compiler Core 调度，Stage 4 支持三路生成策略
- 中段：Bridge Passthrough 28 工具保留为可选外部协议适配层，不是 MCP 核心价值
- 后端：负责测试证据判读 + Run 治理（compare / promote），输出 pass / fail / escalate
- 详见 `Docs/Current/14_MCP_Cognitive_Bridge_Anchor.md`（总口径声明）
...
Phase 4 已完成 Greenfield 自动生成闭环。Phase 5 已完成 Brownfield 最小闭环。Phase 6 已完成”完整 Spec Tree + 可玩 Runtime”阶段。Phase 7 完成”治理闭环 + 多类型扩展”。Phase 8 引入 Skill-First 6 阶段主链，以 MonopolyGame 为垂直切片验证端到端流程。Phase 9 将 Phase 8 的 MCP 占位骨架升级为真实可工作的 stdio server。Phase 10 已将 MCP 从执行层 Bridge 包装重定位为认知桥接层（前端 Stage 1-2 认知分解 + 后端证据裁决），并完成端到端闭环验证。Phase 11 升级为 **Skill-First Design Compiler**：前端扩展至 Stage 1-3（新增 Clarification Gate 与 Skill Graph Planning），后端新增 Run 治理（compare / promote / batch）；主链升级为 7 阶段，Stage 4 Domain Skill Runtime 支持 MCP Agent / LLM Internal / Heuristic Fallback 三路生成；新增 Root Skill Contract / Design Space Discovery / Baseline Domain 模板；Reviewed Handoff 升级到 v3；MCP 工具总数 53（49 正式 + 4 兼容 alias）。
...
| v0.9.0 | MCP 重定位：从执行层 Bridge 包装重定位为认知桥接层（前端 Stage 1-2 认知分解 + 后端证据裁决）；28 工具保留为 Bridge Passthrough（可选外部协议适配层） |
| v1.0.0 | Phase 11 Skill-First Design Compiler：7 阶段主链 + Root Skill Contract + Clarification Gate + Skill Graph + Domain Skill Runtime 三路生成 + Design Space Discovery + Run 治理 + Baseline Domain 模板 + MCP 前端扩展至 Stage 1-3 + MCP 工具总数 53 |
...
```
 ...
 │  ├── GDD/（设计文档）              ├── runs/{run_id}/（Run 产物）    │
 │  ├── Presets/（编译配置）          ├── batches/{batch_id}/（Promote）│
 │  └── Baselines/（人工基线）        ├── Handoffs/（draft / approved） │
 │                                    ├── Reports/（执行报告）          │
 │                                    ├── Snapshots/（状态快照）        │
 │                                    └── Evidence/（阶段证据）         │
 └──────────────┬──────────────────────────────┬───────────────────────┘
 ...
 │                                                                     │
 │  ┌───────────────────────────────────────────────────────────────┐  │
 │  │         MCP 前端：认知桥接层（Stage 1-3）                      │  │
 │  │                                                               │  │
 │  │  compiler_create_session       — Session 创建（含 v1/v2 路由）│  │
 │  │  compiler_root_skill_prepare/save  — Stage 1: Root Skill     │  │
 │  │  compiler_clarification_prepare/save — Stage 2: Clarify Gate │  │
 │  │  compiler_skill_graph_prepare/save — Stage 3: Skill Graph    │  │
 │  │  compiler_get_session_status   — 状态查询                     │  │
 │  │  旧名 alias: intake_prepare/save, plan_prepare/save（兼容）  │  │
 │  │                                                               │  │
 │  │  Agent 通过 MCP 工具驱动认知分解，完成后交付 Compiler Core    │  │
 │  └──────────────────────────┬────────────────────────────────────┘  │
 │                             │ Root Skill + Clarification + Graph    │
 │  ══════════════ MCP 前端边界 │ Stage 4-7 由 Compiler Core 调度 ═══  │
 │                             ▼                                       │
 │  ┌───────────────────────────────────────────────────────────────┐  │
 │  │       Skill Compiler Plane — Compiler Core（Stage 4-7）       │  │
 │  │                                                               │  │
 │  │  Stage 4: Domain Skill Runtime — 三路生成策略                 │  │
 │  │    ├── MCP Agent（主路径：stage4_node_prepare/save 交互）     │  │
 │  │    ├── LLM Internal（pipeline 内嵌 LLM API via llm_client）  │  │
 │  │    └── Heuristic Fallback（*_fallback.py 确定性退化）         │  │
 │  │    含: Discovery → Candidates → Convergence → Fragment Gen   │  │
 │  │    含: Generator-Evaluator 门控 + Trace 审计                  │  │
 │  │                                                               │  │
 │  │  Stage 5: Cross-Domain Review v2 — Constraint 保持性检查     │  │
 │  │    + 跨域 realization 冲突 + Baseline 完整性 + BP 薄层检查   │  │
 │  │                                                               │  │
 │  │  Stage 6: Lowering v2 — Build IR v2                          │  │
 │  │    + naming_resolution_log + provisional_warning + Baseline   │  │
 │  │                                                               │  │
 │  │  Stage 7: Handoff Assembly → Reviewed Handoff v3             │  │
 │  │    + run_id + design_directions + constraint_variant_summary  │  │
 │  │    + baseline_coverage + provisional_items                    │  │
 │  │                                                               │  │
 │  │  Run 治理：run_id 隔离，每轮产物独立存储                      │  │
 │  │  session_version 1.0/2.0 兼容路由                             │  │
 │  │  Legacy 旧链路: intake/routing/generation 仍保留用于回归      │  │
 │  └──────────────────────────┬────────────────────────────────────┘  │
 │                             │ Reviewed Handoff v3                   │
 │                             ▼                                       │
 │  ┌───────────────────────────────────────────────────────────────┐  │
 ...
 │    └──────┬───────┘ └──────┬──────┘ └──────┬──────────┘           │
 │           │                │               │                       │
 │    （MCP Bridge Passthrough 28 工具 = 可选外部协议适配层，         │
 │     保留为外部 Agent 直接操作 UE5 Editor 的备用通道）              │
 │                                                                     │
 │  ┌────────▼────────────────▼───────────────▼───────────────────┐   │
 │  │                                                              │   │
 ...
 └─────────────────────────────┼───────────────────────────────────────┘
    ...
    ┌──────────────────────────▼───────────────────────────────────┐
    │              Gauntlet（C# 测试会话编排）                     │
    │  启动 Editor → 运行测试 → 收集结果 → 产出测试证据           │
    └──────────────────────────┬───────────────────────────────────┘
                               │ 测试证据
    ┌──────────────────────────▼───────────────────────────────────┐
    │         MCP 后端：证据裁决 + Run 治理接口                     │
    │                                                              │
    │  证据裁决：读取测试证据 → Agent 判读 → pass / fail / escalate│
    │  Run 治理：compare_runs / promote_run / create_batch         │
    │  不直接控制 PIE、不模拟输入——仅消费已有证据做裁决            │
    │  fast_mode run 不可 promote；人工确认门仍保留                │
    └─────────────────────────────────────────────────────────────┘
```

> **MCP 定位说明**：MCP 在本框架中的角色是认知桥接层（Cognitive Bridge Layer）。
> 前端（Stage 1-3）负责 Root Skill Contract → Clarification Gate → Skill Graph Planning 的认知分解；
> Stage 4 Domain Skill Runtime 支持 MCP Agent prepare/save 交互（主路径）；
> 中段 Bridge Passthrough 28 工具保留为可选外部协议适配层；
> 后端负责证据裁决（pass / fail / escalate）+ Run 治理（compare / promote / batch）。
> 当前 MCP 工具总数 53（49 正式 + 4 兼容 alias）。
...
| MCP Bridge Passthrough | stdio + 通道 A/B/C | Agent / CLI / IDE 通过 MCP stdio 调用，Server 内部分发到 Bridge 三通道（可选外部协议适配层，非 MCP 核心价值） |
...
│  ═══════▼════════════════▼════════════════════▼═══════════     │
│                                                                 │
│         AgentBridge 框架（本方案 v1.0.0 — Phase 11）             │
│         ════════════════════════════════                         │
│                                                                 │
│    ┌─ MCP 前端认知桥接层（Stage 1-3）───────────────────────┐    │
│    │  Stage 1: Root Skill Contract（能力骨架 + Constraint/ │    │
│    │           Variant + Universal Baseline Merge）         │    │
│    │  Stage 2: Clarification Gate（4 档决策 + provisional  │    │
│    │           传播 + fast_mode 高风险保护）                │    │
│    │  Stage 3: Skill Graph Planning（Gameplay + Baseline   │    │
│    │           Domain Skills 规划）                         │    │
│    │  止于此。Stage 4-7 交由 Compiler Core 调度。           │    │
│    └───────────────────────────┬────────────────────────────┘    │
│                                ▼                                │
│    ┌─ Skill Compiler Plane（编译前端）─────────────────────┐    │
│    │  Compiler Core 调度 Stage 4-7：                        │    │
│    │  Stage 4: Domain Skill Runtime — 三路生成策略          │    │
│    │    （MCP Agent / LLM Internal / Heuristic Fallback）  │    │
│    │    含 Design Space Discovery → Candidates →           │    │
│    │    Convergence → Fragment Generation                   │    │
│    │  Stage 5: Cross-Domain Review v2                      │    │
│    │  Stage 6: Lowering v2 → Build IR v2                   │    │
│    │           （含 naming_resolution_log）                 │    │
│    │  Stage 7: Handoff Assembly → Reviewed Handoff v3      │    │
│    │  输入：GDD / Presets / Project State                  │    │
│    │  输出：Reviewed Handoff v3 + run_id 隔离               │    │
│    └───────────────────────────┬────────────────────────────┘    │
│                                ▼                                │
│    ┌─ Execution Orchestrator Plane（执行编排）──────────────┐    │
...
│                                                                 │
│    ┌─ MCP 后端证据裁决层──────────────────────────────────┐    │
│    │  测试证据读取 → 证据判读 → pass / fail / escalate    │    │
│    │  详见 Docs/Current/14_MCP_Cognitive_Bridge_Anchor.md │    │
│    └──────────────────────────────────────────────────────┘    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
| `root_skill_contract_standard.md` | Root Skill Contract 标准（Phase 11 新增） |
| `universal_baseline_standard.md` | Universal Game Baseline Standard（Phase 11 新增） |
| `baseline_realization_policy.md` | Baseline Realization Policy（Phase 11 新增） |
| `clarification_gate_rules.md` | Clarification Gate 规则（Phase 11 新增） |
| `constraint_variant_policy.md` | Constraint vs Variant 标注策略（Phase 11 新增） |
| `design_space_discovery.md` | Design Space Discovery 与 Realization 协议（Phase 11 新增） |
| `skill_graph_and_domain_skill.md` | Skill Graph 与 Domain Skill（Phase 11 新增） |
| `run_isolation_compare_promote.md` | Run Isolation, Compare & Promote（Phase 11 ��增） |
| `agent_interaction_protocol.md` | Stage 4 Agent 交互协议（Phase 11 新增） |
...
| `Docs/Current/14_MCP_Cognitive_Bridge_Anchor.md` | MCP 认知��接层总口径声明（优先裁决依据） |
| `Docs/Current/15_Skill_Spec_Handoff_Chain.md` | Skill / Static Spec / Dynamic Spec / Reviewed Handoff 四层主链定义 |
| `Docs/Current/16_MCP_Repositioning_Plan.md` | MCP 重定位方案（含完整架构图） |
